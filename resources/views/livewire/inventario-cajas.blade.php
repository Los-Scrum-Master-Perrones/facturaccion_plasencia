<div xmlns:wire="http://www.w3.org/1999/xhtml">
    ...
    <ul class="nav nav-tabs justify-content-center">
        ...
        <li class="nav-item">
            <a class="nav-link" style="color:white;  font-size:12px;" href="importar_c"><strong>Existencia en
                    bodega</strong></a>
        </li>
        <li class="nav-item">
                            ...
                            @foreach ($listacajas as $material)
                                <tr>
                                    ...
                                    <td style=" text-align:center;">
                                        <a onclick="traslardr({{ $material->id }},1,'{{ $material->codigo }}')"
                                            href="#">
                                        ...
                                        <a onclick="traslardr({{ $material->id }},2,'{{ $material->codigo }}')"
                                            href="#">
                                            <abbr title="Entrada">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    fill="currentColor" class="bi bi-basket-fill"
                                                    viewBox="0 0 16 16">
                                                    <path
                                        ...
                                        <a onclick="traslardr({{ $material->id }},3,'{{ $material->codigo }}')"
                                            href="#">
                                            <abbr title="Salida">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    fill="currentColor" class="bi bi-folder-symlink"
                                                    viewBox="0 0 16 16">
                                                    <path
                                    ...
                                    @php
                                        if ($material->existencia < 0) {
                                            DB::update(
                                                'UPDATE lista_cajas SET existencia=0,
                                    faltantes=faltantes + ? WHERE id=?',
                                                [$material->existencia * -1, $material->id],
                                            );
                                    ...
                                    <td style=" text-align:center;">
                                        {{ number_format($material->mal_estado + $material->faltantes + $material->existencia, 0) }}
                                    </td>
                                </tr>
                                <?php
                                $count++;
    ...
    <div wire:ignore class="modal fade" id="material_actualizar" data-backdrop="static" data-keyboard="false"
        tabindex="-1" aria-labelledby="material_actualizar" aria-hidden="true"
        ...
        <div class="modal-dialog modal-lg">
            ...
            <div class="modal-content">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-8 col-md-8">
                                <label for="txt_figuraytipo" class="form-label">Marca</label>
                                <select name="actu_marca" id="actu_marca" style="height:30px;z-index: 999;" required>
                                    <option value="">Marcas</option>
                                    @foreach($marcas_actualizar as $empa)
                                        <option value="{{ $empa->marca }}">
                                            {{ $empa->marca }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="row">
                            <div class="mb-12 col-md-12">
                                <label for="txt_malos" class="form-label">Producto</label>
                                <input class="form-control" name="actu_productos" id="actu_productos">
                            </div>
                        </div>
                        <br>
                        <div class="row">
                            <div class="mb-4 col-md-4">
                                <label for="txt_malos" class="form-label">Mal Estado</label>
                                <input class="form-control" name="actu_mal_estado" id="actu_mal_estado">
                            </div>
                            <div class="mb-4 col-md-4">
                                <label for="txt_malos" class="form-label">Faltantes</label>
                                <input class="form-control" name="actu_faltantes" id="actu_faltantes">
                            </div>
                            <div class="mb-4 col-md-4">
                                <label for="txt_malos" class="form-label">Existencia</label>
                                <input class="form-control" name="actu_existencia" id="actu_existencia">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button onclick="update_material()" class="btn btn-success" data-bs-dismiss="modal"> <span>Guardar</span> </button>
                </div>
            </div>
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            ...
            $(document).ready(function() {
                ...
                select_marca = new TomSelect("#actu_marca", {
                    create: false,
                    sortField: {
                        field: "text",
                        direction: "asc"
            ...
            function editar_material(id, actu_codigo_producto, actu_marca, actu_productoServicio, actu_mal_estado,
                actu_faltantes, actu_existencia) {

                $('#actu_id').val(id);
                $('#actu_productos').val(actu_productoServicio);
                $('#actu_codigo_producto').val(actu_codigo_producto);
                $('#actu_mal_estado').val(actu_mal_estado);
                $('#actu_faltantes').val(actu_faltantes);
                $('#actu_existencia').val(actu_existencia);
                select_marca.setValue(actu_marca);
            ...
            function update_material() {
                @this.update_material({
                            'id': $('#actu_id').val(),
                            'codigo': $('#actu_codigo_producto').val(),
                            'productoServicio': $('#actu_productos').val(),
                            'marca': select_marca.getValue(),
                            'tipo_empaque': 0,
                            'mal_estado': $('#actu_mal_estado').val(),
                            'faltantes': $('#actu_faltantes').val(),
                            'existencia': $('#actu_existencia').val()
                        });
            }
